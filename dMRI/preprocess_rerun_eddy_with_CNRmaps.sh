...
if [ ! -f eddy/eddy_cnr_maps.nii.gz ]; then
  if [ -f dwi_den_unr_eddy.mif.gz ]; then
    ...
    echo "Eddy has not been run in the first place (excluded subject)" 
  fi
else
  echo Eddy has already been re-run and CNR-maps are generated
fi

# Now cleanup by transferring relevant files to topup folder and deleting scratch and eddydir folders
