...
if [ ! -f reg/${meanb0}_2_${t1w}_flirt-bbr.mat ];then 
    ...
    flirt -in ${meanb0}_brain.nii.gz -ref anat/${t1w}_brain.nii.gz -dof 6 -cost bbr -wmseg anat/$wmseg.nii.gz -init reg/tmp.mat -omat reg/${meanb0}_2_${t1w}_flirt-bbr.mat -schedule $FSLDIR/etc/flirtsch/bbr.sch
    rm reg/tmp.mat
fi
# Transform FLIRT registration matrix into MRtrix format
if [ ! -f reg/${meanb0}_2_${t1w}_mrtrix-bbr.mat ];then
     transformconvert reg/${meanb0}_2_${t1w}_flirt-bbr.mat ${meanb0}_brain.nii.gz anat/$t1w.nii.gz flirt_import reg/${meanb0}_2_${t1w}_mrtrix-bbr.mat
fi
...
# T1
mrtransform anat/$t1w.nii.gz -linear reg/${meanb0}_2_${t1w}_mrtrix-bbr.mat anat/${t1w}_space-dwi.mif.gz -inverse
