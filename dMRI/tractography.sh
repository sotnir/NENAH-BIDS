...
# SIFT2-filtering of whole-brain tractogram
    if [ ! -f dwi/tractography/whole_brain_${nbr}_sift2.txt ]; then
        tcksift2 -out_mu dwi/tractography/whole_brain_sift2_mu.txt -act dwi/5tt/$act5tt.mif.gz dwi/tractography/whole_brain_${nbr}_space-anat.tck dwi/csd/$csd.mif.gz dwi/tractography/whole_brain_${nbr}_sift2.txt
    fi
