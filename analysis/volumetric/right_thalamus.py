...
# ============================
os.chdir('/Users/vkumar/Documents/Writings/Pascal/BONN_DATA')  # Change this path to your dataset folder

# ============================
# READ THE DATASET (CHANGE FILE NAME IF USING NEW DATA)
# ============================
# Read CSV file, assuming `;` is the separator and `,` is the decimal format (modify if needed)
data = pd.read_csv("data_freesurfer_for_stats_RIGHT.csv", sep=';', decimal=',')

# ============================
# DATA PREPROCESSING
# ============================
# Replace hyphens in column names with underscores (necessary for compatibility in regression models)
data.columns = data.columns.str.replace('-', '_')

# Exclude rows where the "Age" column has values 1, 2, 3, or 4
# (Modify this list if a different age range should be excluded)
data = data[~data["Age"].isin([1, 2, 3, 4])]

# Separate the dataset into two groups: Controls (0) and Patients (1)
# Ensure the "Group" column exists and correctly labels controls as 0 and patients as 1
...
for structure in structure_names:
    ...
    # ANCOVA model: Predict structure volume based on Group, Age, Sex, and Right Whole Thalamus Volume
    # Ensure 'Right_Whole_thalamus' exists in the dataset, change column name if needed
    model = ols(f"Q('{structure}') ~ Group + Age + Sex + Q('Right_Whole_thalamus')", data=filtered_data).fit()
